...
// Schützt vor HTML/Script in Kommentaren (Stored-XSS)
function escHtml(t) {
  return String(t)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
...
const KommentarStore = {
  ...
  async laden(id) {
    try {
      const res = await fetch(
        `${SUPABASE_URL}/rest/v1/kommentare?station_id=eq.${id}&order=erstellt_am.asc`,
        { headers: SB_HEADERS }
      );
  ...
  async speichern(id, k) {
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/kommentare`, {
        method: 'POST',
        headers: SB_HEADERS,
        body: JSON.stringify({
          station_id: id,
    ...
    } catch (e) { console.error('Kommentar speichern fehlgeschlagen', e); }
    await this.laden(id);
  },
  async freischalten(id, kid) {
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/kommentare?id=eq.${kid}`, {
        method: 'PATCH',
        headers: SB_HEADERS,
        body: JSON.stringify({ freigegeben: true })
      });
    } catch (e) { console.error('Freischalten fehlgeschlagen', e); }
    await this.laden(id);
  },
  async loeschen(id, kid) {
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/kommentare?id=eq.${kid}`, {
        method: 'DELETE',
        headers: SB_HEADERS
  ...
  freigeschaltet(id) { return (this.cache[id] || []).filter(k => k.freigegeben); },
  pending(id)        { return (this.cache[id] || []).filter(k => !k.freigegeben); }
};
...
function aktualisiereAnzahl(stationId) {
  const anzahl = document.getElementById('kommentar-anzahl');
  if (!anzahl) return;
  const n = KommentarStore.freigeschaltet(stationId).length
          + (istAdmin ? KommentarStore.pending(stationId).length : 0);
  anzahl.textContent = n > 0 ? n + (n === 1 ? ' Kommentar' : ' Kommentare') : '';
}
...
async function renderKommentare(stationId) {
  ...
  await KommentarStore.laden(stationId);
  // Falls inzwischen eine andere Station geöffnet wurde: nichts rendern
  ...
  alle.forEach((k, i) => {
    ...
    z.innerHTML = `<div class="notizzettel-text">${escHtml(k.text)}</div>
      <div class="notizzettel-meta"><span>${escHtml(k.name)}</span><span>${escHtml(k.datum || '')}</span></div>
      ${!k.freigegeben && istAdmin ? `
        <div style="display:flex;gap:4px;margin-top:4px;">
          <button class="nz-freischalten" data-id="${k.id}">✓</button>
          <button class="nz-loeschen" data-id="${k.id}">✕</button>
  ...
  zettel.querySelectorAll('.nz-freischalten').forEach(btn => {
    btn.addEventListener('click', async e => {
      e.stopPropagation();
      await KommentarStore.freischalten(stationId, btn.dataset.id);
      await renderKommentare(stationId);
    });
  });
  ...
  zettel.querySelectorAll('.nz-loeschen').forEach(btn => {
    btn.addEventListener('click', async e => {
      e.stopPropagation();
      await KommentarStore.loeschen(stationId, btn.dataset.id);
      await renderKommentare(stationId);
    });
  });

  aktualisiereAnzahl(stationId);
}
...
document.getElementById('kommentar-senden').addEventListener('click', async () => {
  ...
  textEl.placeholder = istAdmin
    ? '✓ Veröffentlicht.'
    : '✓ Danke! Wird nach Freischaltung angezeigt.';
  setTimeout(() => { textEl.placeholder = 'Kommentar schreiben…'; }, 3000);

  if (kommentareSichtbar) await renderKommentare(aktuelleStationId);
});
